...
<div id="sidebar"><a href="../admin.html" class="visible-phone"><i class="icon icon-home"></i> Início</a>
	<ul>
		...
		<li class="active"><a href="../plano_de_contas/plano_contas.html"><i class="icon-list-alt"></i> <span>Plano de Contas</span></a> </li>
					...
					<div class="widget-content nopadding">
						<form  name="form" id="form" novalidate="novalidate" action="" method="post">
							...
							<div class="controls" style="float:right; width:300px;">
								...
								<input style="float:left;" type="text" placeholder="R$" name="val_desc"  class="span3 m-wrap" value=""></input>
							</div>
...
if($titulo == "" || $situacao == "0" || $pagamento=="0" || $data_limite==""){
		...
		if ($lista_produto_id != ""){
			//echo "<script>alert('Passou');</script>";
			   ...
			   $sql= "INSERT INTO planocontas_capa(idPlanoContas_Capa, data,entrada, idForma_Pagamento, idFornecedor, situacao, valor_total, observacao,titulo) VALUES('','$data_limite','N',$pagamento,$fornecedor,'$situacao',' ','$listaobservacao','$titulo')";
							$conn->query($sql);
							...
							//	echo "ID CAPA plano de contas: ".$id_capa;
							}
					...
					$sql4="SELECT SUM(p.preco_final*v.quantidade) as total from planocontas_itens as v INNER JOIN produto as p  WHERE v.idProduto=p.idProduto and v.idPlanoContas_Capa=$id_capa";
						...
						while($row=$result2->fetch_assoc()){
							@$total = $row['total'];
							//echo "Total da Despesa: ".number_format(@$total, 2, ',','.');
							}
